...
#define B_FLAG_FOLLOWERS_DISABLED      FLAG_TOGGLE_FOLLOWERS          // Enables / Disables followers by using a flag. Helpful to disable followers for a period of time.
...
#define OW_FLAG_NO_ENCOUNTER        FLAG_TOGGLE_WILD_ENCOUNTER  // If this flag is set, wild encounters will be disabled.
...
#define OW_NAME_BOX_DEFAULT_WIDTH     16     // Maximum width of what OW_NAME_BOX_USE_DYNAMIC_WIDTH can set. Also the default width when the config above is set to FALSE (or the dynamic width exceeds this value).
